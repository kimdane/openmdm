...
define("org/forgerock/openidm/ui/admin/settings/authentication/AuthenticationModuleDialogView", ["jquery", "underscore", "jsonEditor", "handlebars", "bootstrap-dialog", "org/forgerock/commons/ui/common/main/Configuration", "org/forgerock/openidm/ui/admin/settings/authentication/AuthenticationAbstractView", "org/forgerock/openidm/ui/common/delegates/ConfigDelegate", "org/forgerock/openidm/ui/admin/util/InlineScriptEditor", "org/forgerock/openidm/ui/admin/delegates/ConnectorDelegate", "org/forgerock/commons/ui/common/util/UIUtils", "selectize"], function ($, _, JSONEditor, Handlebars, BootstrapDialog, Conf, AuthenticationAbstractView, ConfigDelegate, InlineScriptEditor, ConnectorDelegate, UIUtils) {
    ...
    var AuthenticationModuleDialogView = AuthenticationAbstractView.extend({
        ...
        render: function render(configs, callback) {
            this.model = _.extend({
                defaultUserRoles: [{ "name": "openidm-admin" }, { "name": "openidm-authorized" }, { "name": "openidm-cert" }, { "name": "openidm-reg" }, { "name": "openidm-task-manager" }]
            }, configs);
            ...
            $.when(this.getResources(), this.getJSONSchema(this.model.config.name)).then(_.bind(function (resources, jsonTemplate) {
                ...
                if (this.model.moduleTemplate.templateName !== "INTERNAL_USER" && this.model.moduleTemplate.templateName !== "STATIC_USER" && _.has(this.model.moduleTemplate.mainSchema, "properties") && _.has(this.model.moduleTemplate.mainSchema.properties, "queryOnResource") && _.has(this.model.moduleTemplate.mainSchema.properties.queryOnResource, "enum")) {
                ...
                this.parentRender(_.bind(function () {
                    ...
                    BootstrapDialog.show({
                        ...
                        onshown: function onshown() {
                            UIUtils.renderTemplate(self.template, self.$el, _.extend({}, Conf.globalData, self.data), function () {
                                self.loadDefaults();
         ...
         * @returns {promise}
         */
        getJSONSchema: function getJSONSchema(moduleName) {
            return $.ajax({
        ...
        makeCustomEditorChanges: function makeCustomEditorChanges() {
            ...
            if (this.model.config.name === "OPENAM_SESSION") {
                ...
                openamDeploymentUrl.unbind("blur").blur(_.bind(function () {
                    this.model.advancedEditor.getEditor('root.openamLoginUrl').setValue(openamLoginUrl.val().replace(openamDeploymentUrl.attr("beforeValue"), openamDeploymentUrl.val()));
                }, this));
            }
        },
        ...
        getConfig: function getConfig(e) {
            ...
            if (!_.isUndefined(editorValues.propertyMapping) && !_.isNull(editorValues.propertyMapping) && !_.isUndefined(editorValues.propertyMapping.userorgroup) && !_.isNull(editorValues.propertyMapping.userorgroup)) {
