...
define("org/forgerock/openidm/ui/common/util/PolicyValidatorsManager", ["jquery", "underscore", "form2js", "org/forgerock/commons/ui/common/main/ValidatorsManager", "org/forgerock/openidm/ui/common/delegates/PolicyDelegate"], function ($, _, form2js, ValidatorsManager, PolicyDelegate) {
    var obj = {};

    // the various policies that we don't want to actually bind in the browser
    obj.excludedClientSidePolicies = ["valid-type", "re-auth-required"];
    // related to above, the various requirements that we don't want to show in the browser
    obj.excludedClientSideRequirements = ["VALID_TYPE", "REAUTH_REQUIRED"];
            ...
            if (policyFunctionObject.policyFunction && policyFunctionObject.policyFunction.length) {
                ValidatorsManager.configuration.validators[policyFunctionObject.policyId] = {
                    ...
                    validator: function validator(container, element, callback) {
                        ...
                        }, form2js(container[0], '.', true, _.curry(obj.nodeCallbackHandler)(container.hasClass("jsonEditor"))), nodeMap.value, element.data("validatorParams-" + policyFunctionObject.policyId) || {}, nodeMap.name);
